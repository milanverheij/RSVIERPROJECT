CREATE DATABASE 'RSVIERPROJECTDEEL3.fdb' DEFAULT CHARACTER SET utf8;
...
CREATE UNIQUE INDEX klant_id_UNIQUE ON KLANT (klantId);
...
RECREATE TABLE ADRES (
  adresId INT NOT NULL PRIMARY KEY,
...
CREATE UNIQUE INDEX adres_id_UNIQUE ON ADRES (adresId);
...
CREATE INDEX klant_id_idx ON BESTELLING (klantId);
...
RECREATE TABLE ARTIKEL (
  ...
  inAssortiment CHAR(1) DEFAULT 1);

CREATE INDEX prijs_id_prijs_idx ON ARTIKEL (prijs_id);
